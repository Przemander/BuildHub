...
use crate::{
    ...
    metricss::rate_limiter_metrics::{
        redis as rate_redis, request as rate_request, time_rate_limit_check,
    },
    utils::{
        error_new::ApiError,
        log_new::Log,
    ...
    fn call(&mut self, req: Request<Body>) -> Self::Future {
        ...
        Box::pin(
            async move {
                ...
                let rate_check_result =
                    check_and_increment_rate_limit(&redis, &key, max_attempts, window_secs).await;
                ...
                let allowed = match rate_check_result {
                    ...
                    Err(cache_err) => {
                        ...
                        let error_category = if error_string.contains("connection")
                ...
                if !allowed {
                    Log::event(
                        ...
                        "RateLimiter",
                        &format!(
                            "Rate limit exceeded for key prefix: {}",
