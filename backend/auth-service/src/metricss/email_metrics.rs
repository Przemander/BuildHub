...
pub(crate) fn init_email_metrics() {
    ...
    log_info!("Metrics", "Email metrics initialized (production-ready with full standardization)", "email_metrics_init");
}
...
/// Records email processing duration directly (for cases where timer wasn't used)
#[allow(dead_code)]
pub fn record_email_duration(email_type: &str, duration_seconds: f64) {
    EMAIL_DURATION
        .with_label_values(&[email_type])
...
pub mod failure_types {
    pub const SMTP_CONNECTION: &str = "smtp_connection";
    pub const SMTP_AUTH: &str = "smtp_auth";
    pub const SMTP_TIMEOUT: &str = "smtp_timeout";
...
mod tests {
    ...
    fn test_email_metrics_initialization() {
        ...
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::SUCCESS]).get(), 0.0);
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::SMTP_CONNECTION, email_types::ACTIVATION]).get(), 0.0);
        assert_eq!(EMAIL_DURATION.with_label_values(&[email_types::ACTIVATION]).get_sample_count(), 0);
    }
    ...
    #[test]
    fn test_direct_duration_recording() {
        init_email_metrics();
        ...
        record_email_duration(email_types::PASSWORD_RESET, 1.5);
    ...
    fn test_business_helper_consistency() {
        ...
        record_activation_failure_detailed(failure_types::SMTP_CONNECTION);
        record_password_reset_failure_detailed(failure_types::INVALID_ADDRESS);
        
        // Verify operations were recorded
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::SUCCESS]).get(), 1.0);
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::FAILURE]).get(), 2.0); // General + detailed
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::PASSWORD_RESET, results::SUCCESS]).get(), 1.0);
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::PASSWORD_RESET, results::FAILURE]).get(), 2.0); // General + detailed
        
        // Verify detailed failures were recorded
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::SMTP_CONNECTION, email_types::ACTIVATION]).get(), 1.0);
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::INVALID_ADDRESS, email_types::PASSWORD_RESET]).get(), 1.0);
    }
    ...
    fn test_comprehensive_email_types_and_failures() {
        ...
        record_email_failure(failure_types::SMTP_CONNECTION, email_types::ACTIVATION);
        record_email_failure(failure_types::SMTP_AUTH, email_types::ACTIVATION);
        record_email_failure(failure_types::SMTP_TIMEOUT, email_types::PASSWORD_RESET);
        ...
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::SUCCESS]).get(), 1.0);
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::PASSWORD_RESET, results::FAILURE]).get(), 1.0);
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::SMTP_AUTH, email_types::ACTIVATION]).get(), 1.0);
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::CONFIGURATION, email_types::PASSWORD_RESET]).get(), 1.0);
    }
    ...
    fn test_production_email_patterns() {
        ...
        record_email_duration(email_types::ACTIVATION, 0.8);
        record_email_duration(email_types::ACTIVATION, 1.2);
        record_email_duration(email_types::PASSWORD_RESET, 1.5);
        
        // Verify realistic ratios
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::SUCCESS]).get(), 10.0);
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::PASSWORD_RESET, results::SUCCESS]).get(), 5.0);
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::FAILURE]).get(), 2.0); // 2 detailed failures
        
        // Verify failure categorization
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::SMTP_CONNECTION, email_types::ACTIVATION]).get(), 1.0);
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::TOKEN_STORAGE, email_types::ACTIVATION]).get(), 1.0);
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::INVALID_ADDRESS, email_types::PASSWORD_RESET]).get(), 1.0);
        
        // Verify duration tracking
        assert_eq!(EMAIL_DURATION.with_label_values(&[email_types::ACTIVATION]).get_sample_count(), 2);
        assert_eq!(EMAIL_DURATION.with_label_values(&[email_types::PASSWORD_RESET]).get_sample_count(), 1);
    }
    ...
    fn test_edge_cases_and_validation() {
        ...
        record_email_duration("", 0.0); // Zero duration
        record_email_duration(email_types::ACTIVATION, -1.0); // Negative duration (Prometheus handles this)
        
        // These should not panic due to core infrastructure protection
        // Just verify we can continue operating
        record_activation_success();
        assert!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::SUCCESS]).get() >= 1.0);
    }

    #[test]
    fn test_standardized_buckets_usage() {
        init_email_metrics();
        
        // Test that we're using standardized buckets
        record_email_duration(email_types::ACTIVATION, 0.05); // Below first bucket
        record_email_duration(email_types::ACTIVATION, 0.5);  // In middle buckets
        record_email_duration(email_types::ACTIVATION, 2.0);  // Higher bucket
        record_email_duration(email_types::ACTIVATION, 10.0); // Even higher
        
        // Verify all durations were recorded
        assert_eq!(EMAIL_DURATION.with_label_values(&[email_types::ACTIVATION]).get_sample_count(), 4);
        
        // Verify sum is reasonable (should be 12.55)
        let sum = EMAIL_DURATION.with_label_values(&[email_types::ACTIVATION]).get_sample_sum();
        assert!((sum - 12.55).abs() < 0.01);
    }
    ...
    fn test_type_safety_constants() {
        ...
        assert_eq!(failure_types::SMTP_CONNECTION, "smtp_connection");
        assert_eq!(failure_types::SMTP_AUTH, "smtp_auth");
        assert_eq!(failure_types::SMTP_TIMEOUT, "smtp_timeout");
        ...
        record_email_operation(email_types::ACTIVATION, results::SUCCESS);
        record_email_failure(failure_types::SMTP_CONNECTION, email_types::PASSWORD_RESET);
        
        assert_eq!(EMAIL_OPERATIONS.with_label_values(&[email_types::ACTIVATION, results::SUCCESS]).get(), 1.0);
        assert_eq!(EMAIL_FAILURES.with_label_values(&[failure_types::SMTP_CONNECTION, email_types::PASSWORD_RESET]).get(), 1.0);
    }
}